...
async def get_admins(current_admin: Dict[str, Any] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        current_role = next((u["role"] for u in admins if u["email"] == current_admin["email"]), "admin")
        return {
...
async def create_admin(
    ...
    try:
        ...
        if success:
            ...
            return {"status": "success", "message": f"Admin {request.email} added successfully with role {request.role}", "admin_email": request.email, "role": request.role}
        raise HTTPException(status_code=400, detail=f"Failed to add admin {request.email}. They may already exist.")
...
async def update_admin_role(
    ...
    try:
        ...
        if success:
            ...
            return {"status": "success", "message": f"Admin {email} role updated to {request.role}", "admin_email": email, "new_role": request.role}
        raise HTTPException(status_code=404, detail=f"Admin {email} not found or update failed")
...
async def remove_admin(
    ...
    try:
        ...
        from app.services.admin_service import admin_service
        success = await admin_service.remove_admin_user(email=email, removed_by_email=current_admin["email"])
        if success:
            logger.info(f"Super admin {current_admin['email']} removed admin {email}")
            return {"status": "success", "message": f"Admin {email} has been deactivated", "admin_email": email, "action": "deactivated"}
        raise HTTPException(status_code=404, detail=f"Admin {email} not found or removal failed")
...
async def delete_admin_permanently(
    ...
    try:
        ...
        from app.services.admin_service import admin_service
        success = await admin_service.delete_admin_user(email=email, deleted_by_email=current_admin["email"])
        if success:
            logger.info(f"Super admin {current_admin['email']} permanently deleted admin {email}")
            return {"status": "success", "message": f"Admin {email} has been permanently deleted from database", "admin_email": email, "action": "permanently_deleted"}
        raise HTTPException(status_code=404, detail=f"Admin {email} not found or deletion failed")
