...
def get_schedule_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        visible_schedule_sheets = [
            ...
            if not s["hidden"] and s["date"] is not None
        ]
        return {
            "status": "success",
            "details": {
...
async def rotate_schedule_sheets(
    ...
    try:
        with get_db_session() as db:
            if display_weeks is not None:
                if display_weeks < 1 or display_weeks > 12:
                    raise HTTPException(status_code=400, detail="display_weeks must be between 1 and 12")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to rotate schedule sheets: {str(e)}")
