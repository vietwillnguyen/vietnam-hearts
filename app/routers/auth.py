...
async def auth_callback(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to check admin status for {user_email}: {admin_check_error}"
            )
        ...
        logger.info(
            f"Access token present: {'access_token' in result.get('session', {})}"
        )
        logger.info(
            f"Refresh token present: {'refresh_token' in result.get('session', {})}"
        )
            ...
            users.append(
                {
                    ...
                    "last_sign_in": admin_user.last_login.isoformat()
...
async def auth_health_check() -> dict[str, Any]:
    ...
    return {
        ...
        + (" (using mock auth)" if not all_configured else ""),
