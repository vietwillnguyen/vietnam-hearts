...
class AdminService:
                ...
                admin_users.append(
                    AdminUser(
                        ...
                        last_login=datetime.fromisoformat(
                            row["last_login"].replace("Z", "+00:00")
                        )
                        if row["last_login"]
                        else None,
                    )
    ...
    async def add_admin_user(
        self, email: str, role: str = "admin", added_by_email: str = None
    ) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self.admin_supabase.table("admin_users")
                    .insert({"email": email.lower(), "role": role, "is_active": True})
                    .execute()
                ),
                timeout=10.0,
            )
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self.admin_supabase.table("admin_users")
                    .select("role")
                    .eq("email", removed_by_email.lower())
                    .eq("is_active", True)
                    .execute()
                ),
                timeout=10.0,
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self.admin_supabase.table("admin_users")
                    .select("role")
                    .eq("email", deleted_by_email.lower())
                    .eq("is_active", True)
                    .execute()
                ),
                timeout=10.0,
    ...
    async def update_admin_role(
        self, email: str, new_role: str, updated_by_email: str
    ) -> bool:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self.admin_supabase.table("admin_users")
                    .select("role")
                    .eq("email", updated_by_email.lower())
                    .eq("is_active", True)
                    .execute()
                ),
                timeout=10.0,
