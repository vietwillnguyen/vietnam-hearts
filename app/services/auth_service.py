...
class AuthService:
    ...
    def __init__(self):
        ...
        self._admin_cache: dict[str, dict[str, Any]] = {
            email: {"is_admin": True, "timestamp": time.time(), "source": "environment"}
    ...
    def _cleanup_cache(self):
        ...
        if current_time - self._last_cache_cleanup > 60:  # Cleanup every minute
            expired_keys = [
                key
                for key, data in self._admin_cache.items()
    ...
    async def sign_in_with_google(
        ...
        try:
            ...
            auth_url = self.supabase.auth.sign_in_with_oauth(
                {"provider": "google", "options": {"redirect_to": final_redirect_to}}
            )

            logger.info("Generated Google OAuth URL for sign-in")
            return {
    ...
    async def _get_user_from_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_sign_in": user.last_sign_in_at.isoformat()
    ...
    async def _is_admin_cached(self, email: str) -> bool:
        ...
        if email in self._admin_cache:
            ...
            if time.time() - cache_data["timestamp"] < self._cache_ttl:
                logger.debug(
                    f"Admin status from cache: {email} = {cache_data['is_admin']}"
                )
    ...
    async def _update_last_login_async(self, email: str):
        ...
        try:
            ...
            await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: self.admin_supabase.rpc(
                        "update_admin_last_login", {"user_email": email}
                    ).execute()
                ),
    ...
    async def sign_out(self, access_token: str | None = None) -> dict[str, str]:
    ...
    def clear_admin_cache(self, email: str | None = None):
