...
class MessageSender:
    ...
    def send_quick_reply(
        self, recipient_id: str, text: str, quick_replies: list
    ) -> bool:
        """
    ...
    def _send_message(self, payload: dict[str, Any]) -> bool:
        ...
        try:
            ...
            response = requests.post(
                self.api_url, json=payload, params=params, timeout=10
            )
    ...
    def get_page_info(self) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Retrieved page info: {page_info.get('name', 'Unknown')} (ID: {page_info.get('id', 'Unknown')})"
                )
                return page_info
            else:
                logger.error(
                    f"Failed to get page info: {response.status_code} - {response.text}"
                )
                return None
