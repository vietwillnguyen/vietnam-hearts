...
from app.config import API_URL, ENVIRONMENT
...
def setup_cors(app: FastAPI) -> None:
    ...
    if ENVIRONMENT in ["development", "test"]:
        ...
        logger.info(
            f"🔓 CORS configured for {ENVIRONMENT} - allowing localhost origins"
        )
    else:
        ...
        logger.info(
            "🔒 CORS configured for production - restricting to allowed origins"
        )
    ...
    logger.info(
        f"✅ CORS middleware configured with {len(allowed_origins)} allowed origins"
    )
