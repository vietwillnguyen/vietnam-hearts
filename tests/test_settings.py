...
EXPECTED_DEFAULTS = {
    ...
    "CRON_SEND_WEEKLY_REMINDERS": "0 9 * * 1",
    "CRON_ROTATE_SCHEDULE": "0 0 * * 0",
}
...
class TestCronDefaultSettings:
    def test_all_cron_keys_created_after_init(self, db):
        ...
        for cron_key in CRON_KEYS:
            assert cron_key in keys, f"{cron_key} missing after initialize_default_settings"
...
class TestCronSettingsPersistence:
    ...
    def test_update_does_not_affect_other_cron_keys(self, db):
        ...
        assert get_setting(db, "CRON_SEND_WEEKLY_REMINDERS") == EXPECTED_DEFAULTS["CRON_SEND_WEEKLY_REMINDERS"]
