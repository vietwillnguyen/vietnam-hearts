...
class TestDatabaseIntegrity:
    ...
    def test_volunteer_creation_and_retrieval(self, test_db):
        ...
        retrieved_volunteer = (
            test_db.query(VolunteerModel).filter_by(email="dbtest@example.com").first()
        )

        assert retrieved_volunteer is not None
        assert retrieved_volunteer.name == "Database Test Volunteer"
        assert retrieved_volunteer.is_active is True
    ...
    def test_email_communication_logging(self, test_db, mock_volunteer):
        ...
        retrieved_comm = (
            ...
            .filter_by(volunteer_id=mock_volunteer.id, email_type="test_email")
            .first()
        )
