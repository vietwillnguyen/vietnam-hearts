...
class TestRotationNaming:
    def test_new_sheets_created_with_ddmmyyyy_titles(self, service):
        ...
        created_dates = [c.args[1] for c in service.create_sheet_from_template.call_args_list]
        assert created_dates[0].date() == next_monday().date()
        expected_titles = [format_schedule_sheet_title(d) for d in created_dates]
        assert all(t.startswith("Schedule ") and t.count("/") == 2 for t in expected_titles)
    ...
    def test_legacy_titled_sheet_matched_by_date_and_renamed(self, service):
        ...
        assert service.rename_sheet.call_args.args[1] == format_schedule_sheet_title(monday)
        # Made visible
        assert (55, False) in [(c.args[0], c.args[1]) for c in service.set_sheet_visibility.call_args_list]
    ...
    def test_canonical_titled_sheet_not_renamed(self, service):
        monday = next_monday()
        canonical = sheet_props(format_schedule_sheet_title(monday), sheet_id=60, hidden=True)
