...
class TestFAQHandling:
    ...
    def create_webhook_payload(
        self, base_payload, message_text=None, postback_payload=None
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ["vietnam hearts", "volunteer", "english", "teaching"],
            ),
            (
            ...
            (
                ...
                ["certificate", "required", "teach", "volunteer"],
            ),
            # Location and timing
            (
                "Where are you located?",
                ["binh thanh", "ho chi minh", "vietnam", "location"],
            ),
            ...
            (
                ...
                ["children", "students", "underprivileged", "vietnam"],
            ),
            ("How old are the children?", ["age", "children", "students", "young"]),
            (
                "How many students do you have?",
                ["students", "number", "class size", "children"],
            ),
            # Contact and support
            ...
            (
                ...
                ["english", "language", "teach", "vietnamese"],
            ),
            ("Is this free?", ["free", "cost", "volunteer", "donation"]),
            (
                "How long have you been operating?",
                ["operating", "years", "established", "history"],
            ),
        ],
    )
    def test_faq_responses(
        ...
        webhook_payload = self.create_webhook_payload(
            webhook_base_payload, message_text=message
        )
        ...
        assert any(
            keyword in response_lower for keyword in expected_response_contains
        ), f"Response '{response_text}' should contain one of {expected_response_contains}"

    @pytest.mark.parametrize(
        "postback_payload,expected_response_contains",
        [
    ...
    def test_postback_responses(
        ...
        webhook_payload = self.create_webhook_payload(
            webhook_base_payload, postback_payload=postback_payload
        )
    ...
    def test_contextual_responses(
        ...
        if expected_behavior == "should_respond_in_vietnamese":
            ...
            assert any(char in response_text for char in vietnamese_chars) or any(
                word in response_text.lower()
                for word in ["xin chào", "cảm ơn", "tôi", "bạn"]
            )
            ...
            assert any(
                word in response_text.lower()
                for word in ["thank you", "appreciate", "welcome"]
            )
            ...
            assert any(
                word in response_text.lower()
                for word in ["australia", "location", "country"]
            )
            ...
            assert any(
                word in response_text.lower()
                for word in ["teacher", "experience", "teaching", "retired"]
            )
            ...
            assert any(
                word in response_text.lower()
                for word in ["weekend", "schedule", "time", "available"]
            )

    @pytest.mark.parametrize(
        "message,expected_fallback",
        [
            # Unclear questions
    ...
    def test_conversation_flow(
        self, client: TestClient, mock_message_sender, webhook_base_payload
    ):
        """Test a complete conversation flow"""
        # Start conversation
        start_payload = self.create_webhook_payload(
            webhook_base_payload, message_text="Hello"
        )
        response = client.post("/webhook/messenger", json=start_payload)
        assert response.status_code == 200

        # Ask about volunteering
        volunteer_payload = self.create_webhook_payload(
            webhook_base_payload, message_text="I want to volunteer"
        )
        response = client.post("/webhook/messenger", json=volunteer_payload)
        assert response.status_code == 200

        # Ask about location
        location_payload = self.create_webhook_payload(
            webhook_base_payload, message_text="Where are you located?"
        )
        ...
        assert (
            len(set(responses)) >= 2
        ), "Should provide different responses to different questions"
    ...
    def test_error_handling(self, client: TestClient, webhook_base_payload):
        ...
        empty_payload = self.create_webhook_payload(
            webhook_base_payload, message_text=""
        )
        ...
        long_message = "A" * 1000
        long_payload = self.create_webhook_payload(
            webhook_base_payload, message_text=long_message
        )
    ...
    def test_response_quality(
        ...
        for response_text in responses:
            ...
            assert (
                response_text.strip()
            ), f"Response is empty or whitespace: '{response_text}'"

            # Should not contain obvious errors
            assert (
                "error" not in response_text.lower()
                or "error" in response_text.lower() in ["no error", "error handling"]
            )
